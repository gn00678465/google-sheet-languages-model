...
Issues and specs are local markdown files under `docs/specs/<NNNN>-<slug>/` (`spec.md` + `issues/NN-*.md`). Never create GitHub issues. See `docs/agents/issue-tracker.md`.

### Triage labels

Recorded as a `status:` frontmatter field (`needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`, `wontfix`), not GitHub labels. See `docs/agents/triage-labels.md`.

### Domain docs

Single-context: `CONTEXT.md` at the repo root plus ADRs in `docs/adr/`. Research notes live in `docs/research/`. See `docs/agents/domain.md`.
